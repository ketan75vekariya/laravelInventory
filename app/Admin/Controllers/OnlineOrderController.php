<?php

namespace App\Admin\Controllers;

use App\Models\OnlineOrder;
use Encore\Admin\Controllers\AdminController;
use Encore\Admin\Form;
use Encore\Admin\Grid;
use Encore\Admin\Show;

class OnlineOrderController extends AdminController
{
    /**
     * Title for current resource.
     *
     * @var string
     */
    protected $title = 'OnlineOrder';

    /**
     * Make a grid builder.
     *
     * @return Grid
     */
    protected function grid()
    {
        $grid = new Grid(new OnlineOrder());

        $grid->column('id', __('Id'));
        $grid->column('status', __('Status'));
        $grid->column('totalPrice', __('TotalPrice'));
        $grid->column('orderDate', __('OrderDate'));
        $grid->column('PaymentMethod', __('PaymentMethod'));
        $grid->column('note', __('Note'));
        $grid->column('created_at', __('Created at'));
        $grid->column('updated_at', __('Updated at'));
        $grid->column('package_id', __('Package id'));
        $grid->column('user_id', __('User id'));
        $grid->column('customer_id', __('Customer id'));

        return $grid;
    }

    /**
     * Make a show builder.
     *
     * @param mixed $id
     * @return Show
     */
    protected function detail($id)
    {
        $show = new Show(OnlineOrder::findOrFail($id));

        $show->field('id', __('Id'));
        $show->field('status', __('Status'));
        $show->field('totalPrice', __('TotalPrice'));
        $show->field('orderDate', __('OrderDate'));
        $show->field('PaymentMethod', __('PaymentMethod'));
        $show->field('note', __('Note'));
        $show->field('created_at', __('Created at'));
        $show->field('updated_at', __('Updated at'));
        $show->field('package_id', __('Package id'));
        $show->field('user_id', __('User id'));
        $show->field('customer_id', __('Customer id'));

        return $show;
    }

    /**
     * Make a form builder.
     *
     * @return Form
     */
    protected function form()
    {
        $form = new Form(new OnlineOrder());

        $form->text('status', __('Status'));
        $form->decimal('totalPrice', __('TotalPrice'));
        $form->date('orderDate', __('OrderDate'))->default(date('Y-m-d'));
        $form->text('PaymentMethod', __('PaymentMethod'));
        $form->textarea('note', __('Note'));
        $form->number('package_id', __('Package id'));
        $form->number('user_id', __('User id'));
        $form->number('customer_id', __('Customer id'));

        return $form;
    }
}
